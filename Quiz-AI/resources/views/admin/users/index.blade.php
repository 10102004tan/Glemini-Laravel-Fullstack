...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="bg-primary border-b dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600 text-center">
                ...
                <td class="px-6 py-4">
                    <a href="#" class="font-medium text-blue-600 dark:text-blue-500 hover:underline editUserButton" data-id="{{$user->id}}" data-name="{{$user->name}}" data-email="{{$user->email}}">Chỉnh sửa</a>
                </td>
            </tr>
            @endforeach
        </tbody>
...
<div id="editUserModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
        ...
        <form id="editUserForm" action="{{ route('users.update', ['id' => $user->id]) }}" method="POST">
            @csrf
            @method('PUT')
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            if(data.success) {
                ...
                const newRow = `
                    <tr class="bg-primary border-b dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600 text-center">
                        ...
                        <td class="px-6 py-4">
                            <a href="#" class="font-medium text-blue-600 dark:text-blue-500 hover:underline editUserButton" data-id="${data.user.id}" data-name="${data.user.name}" data-email="${data.user.email}">Chỉnh sửa</a>
                        </td>
                    </tr>
                `;
    ...
    document.getElementById('editUserForm').addEventListener('submit', function(event) {
        ...
        const actionUrl = this.action.replace('users/update', `users/${userId}`);
        ...
        .then(data => {
            if(data.success) {
                ...
                userRow.querySelector('td:nth-child(2) span').innerText = data.user.name;
